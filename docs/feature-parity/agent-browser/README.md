...
Latest update focus: the Firefox WebExtension backend now covers the core ref/selector command path, including CSS/text=/xpath= selectors, `get`, `is`, `type`, keyboard text insertion, explicit `select`/`check`/`uncheck`, left/right/container scroll, tab aliases, navigation commands, inline `batch`, cookies/storage, visible-viewport screenshot options, and structured JSON envelopes. Features that still rely on CDP, cloud providers, downloads, network routing, dashboards/streaming, video/trace/profiling, auth/state vaults, React/vitals, or iOS remain unavailable or only best-effort.
