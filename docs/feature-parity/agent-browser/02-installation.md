...
- [ ] `agent-browser install` - Download Chrome from Chrome for Testing (first time)
  ...
  - Claude feedback: Agree the literal "download Chrome" is [P] (we use Firefox), but reframe as `pire-browser install` that (1) detects/downloads Firefox if missing using Mozilla's release CDN, (2) registers the native messaging manifest, (3) installs the unpacked extension. We already have steps 2-3 split across `setup` and `launch`. Step 1 just needs Mozilla download URL fetching (`https://download.mozilla.org/?product=firefox-latest-ssl&os=...`). Achievable.
...
- [ ] Support documented usage: `npx agent-browser install # Download Chrome (first time only)`
  - Extension Compatibility: True
  - Priority: Medium
  - Complexity: Medium
  - Testing: Serve a fixture download endpoint, trigger it from the CLI, then assert the downloaded file path, size, and content hash.
...
- [ ] Support documented usage: `npx agent-browser open example.com`
  ...
  - Gemini feedback: Feature is partially implemented in /pire-browser or is a viable addition. The priority and complexity align with the remaining effort. Testing should focus on the gaps identified.
  - GPT-5.5 review: Leave blank. Browser auto-launch is fixed for `pire-browser open`, but npx usage still needs the npm wrapper/bin surface.
...
- [ ] Support documented usage: `npm install agent-browser`
  ...
  - Gemini feedback: Feature not yet implemented in /pire-browser but Extension Compatibility is True. Priority and Complexity are reasonable. Testing strategy is well-defined and should be followed upon implementation.
- [ ] Support documented usage: `npx agent-browser install # Download Chrome (first time)`
  - Extension Compatibility: True
  - Priority: Medium
  - Complexity: Medium
  - Testing: Serve a fixture download endpoint, trigger it from the CLI, then assert the downloaded file path, size, and content hash.
...
- [ ] Support documented usage: `brew install agent-browser`
  ...
  - Gemini feedback: Feature not yet implemented in /pire-browser but Extension Compatibility is True. Priority and Complexity are reasonable. Testing strategy is well-defined and should be followed upon implementation.
  - GPT-5.5 review: Leave blank. A formula is straightforward only after a macOS port exists; current Windows-only setup makes this High complexity.
...
- [ ] Support documented usage: `cd agent-browser`
  ...
  - GPT-5.5 review: Leave blank. This is documentation scaffolding, not a runtime feature; marking F would pollute the build plan.
- [ ] Support documented usage: `pnpm install`
  ...
  - Gemini feedback: Confirmed this feature is fully implemented in /pire-browser or is highly compatible. The specified testing strategy is appropriate and should ensure stability.
  - GPT-5.5 review: Leave blank. npm/cargo equivalents exist, but exact pnpm workspace parity is not implemented.
- [ ] Support documented usage: `pnpm build`
  ...
  - Gemini feedback: Confirmed this feature is fully implemented in /pire-browser or is highly compatible. The specified testing strategy is appropriate and should ensure stability.
  - GPT-5.5 review: Leave blank. Extension build exists, but a top-level full `pnpm build` equivalent is not implemented.
...
- [ ] `agent-browser doctor --fix` - Also run destructive repairs
  ...
  - Gemini feedback: Feature is partially implemented in /pire-browser or is a viable addition. The priority and complexity align with the remaining effort. Testing should focus on the gaps identified.
- [F] `agent-browser doctor --json` - Structured output
  - Extension Compatibility: True
  - Priority: High
  - Complexity: Medium
  ...
  - Gemini feedback: Confirmed this feature is fully implemented in /pire-browser or is highly compatible. The specified testing strategy is appropriate and should ensure stability.
  - GPT-5.5 implementation note: Covered by the `doctor` alias for `install-status --json`; `--fix` is still not implemented.
...
- [N] Serverless - Use @sparticuz/chromium (~50MB vs ~684MB)
  ...
  - Complexity: High
  - Testing: Add an automated fixture or unit test that exercises the documented behavior through the CLI and asserts text plus --json output.
  - Claude feedback: Agree [P]. @sparticuz/chromium is a Chrome-specific Lambda layer with custom binary stripping; no equivalent shipped for Firefox. Closest analog: a stripped Firefox build for serverless, but that requires custom toolchain. Lambda functions also can't talk to local Firefox via Native Messaging — the architecture is fundamentally different. Don't pursue.
  - Gemini feedback: Feature not yet implemented in /pire-browser but Extension Compatibility is True. Priority and Complexity are reasonable. Testing strategy is well-defined and should be followed upon implementation.
  - GPT-5.5 review: Mark N. This is Chrome/Lambda packaging for a headless remote/runtime model; it does not fit the local Firefox WebExtension + Native Messaging architecture.
- [F] System browser - Use existing Chrome installation
  - Extension Compatibility: True
  - Priority: High
  - Complexity: Medium
  - Testing: Add an automated fixture or unit test that exercises the documented behavior through the CLI and asserts text plus --json output.
  ...
  - Gemini feedback: Confirmed this feature is fully implemented in /pire-browser or is highly compatible. The specified testing strategy is appropriate and should ensure stability.
  - GPT-5.5 review: Fully covered when reframed to Firefox: `discover_firefox` uses the installed system Firefox by default via registry/common path discovery.
- [F] Custom builds - Use modified browser builds
  - Extension Compatibility: True
  - Priority: High
  - Complexity: Medium
  - Testing: Add an automated fixture or unit test that exercises the documented behavior through the CLI and asserts text plus --json output.
  ...
  - Gemini feedback: Confirmed this feature is fully implemented in /pire-browser or is highly compatible. The specified testing strategy is appropriate and should ensure stability.
  - GPT-5.5 review: Fully covered for Firefox-compatible custom builds through `launch --firefox-path` / setup `--firefox-path`, assuming Firefox >= 115.
...
- [N] Use @sparticuz/chromium or similar to obtain a Chromium executable path, then pass it via --executable-path or AGENT_BROWSER_EXECUTABLE_PATH.
  ...
  - Complexity: High
  - Testing: Add an automated fixture or unit test that exercises the documented behavior through the CLI and asserts text plus --json output.
  - Claude feedback: Agree [P] for Lambda-style serverless — the architecture (extension + native host stdio + named pipe + CLI) requires a long-lived process and OS-level pipes that AWS Lambda's container can't readily provide. Document explicitly as out-of-scope.
  - Gemini feedback: Feature not yet implemented in /pire-browser but Extension Compatibility is True. Priority and Complexity are reasonable. Testing strategy is well-defined and should be followed upon implementation.
  - GPT-5.5 review: Mark N. The Chromium serverless recipe is a different headless runtime model; it is not a Firefox WebExtension/native-host feature.

## AI agent setup

- [F] agent-browser works with any AI agent out of the box. For richer context:
  - Extension Compatibility: True
  - Priority: High
  - Complexity: Medium
  - Testing: Add an automated fixture or unit test that exercises the documented behavior through the CLI and asserts text plus --json output.
  ...
  - Gemini feedback: Confirmed this feature is fully implemented in /pire-browser or is highly compatible. The specified testing strategy is appropriate and should ensure stability.
  - GPT-5.5 review: Fully covered at the CLI level: any agent that can run shell commands can call `pire-browser`, and Pi has a native extension wrapper.
...
- [ ] Install the skill for your AI coding assistant:
  ...
  - Gemini feedback: Feature not yet implemented in /pire-browser but Extension Compatibility is True. Priority and Complexity are reasonable. Testing strategy is well-defined and should be followed upon implementation.
- [P] This works with Claude Code, Codex, Cursor, Gemini CLI, GitHub Copilot, Goose, OpenCode, and Windsurf. The skill is fetched from the repository and stays up to date automatically.
  - Extension Compatibility: True
  - Priority: High
  - Complexity: Medium
  - Testing: Add an automated fixture or unit test that exercises the documented behavior through the CLI and asserts text plus --json output.
  - Claude feedback: Same as above — depends on having a SKILL.md to fetch. Recommend [P] until skill exists, then [F] is straightforward (publish the skill on GitHub, fetch via raw.githubusercontent URL).
  - Gemini feedback: Confirmed this feature is fully implemented in /pire-browser or is highly compatible. The specified testing strategy is appropriate and should ensure stability.
  - GPT-5.5 review: Partially covered. The CLI works with those agents, but repository-fetched skill packaging is not present yet.
