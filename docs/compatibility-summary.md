...
| Network and configuration | Partial | Domain allowlists, headers, session-memory HTTP Basic credentials, best-effort offline request blocking, best-effort viewport/device settings, network diagnostics, metadata-only HAR start/stop/export, route mocks, and config defaults are available with documented limits. |
