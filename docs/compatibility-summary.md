...
| MCP agent integration | Supported | `pire-browser mcp --tools core` exposes a compact inspect-before-act stdio MCP workflow, including agent-browser-style typed wait tools. Agent-browser-style profiles `network`, `state`, `debug`, `tabs`, `mobile`, `react`, and `all` can be selected or comma-combined to expose broader typed tools on demand. |
