...
| MCP agent integration | Supported | `pire-browser mcp --tools core` exposes the main browser workflow as stdio MCP tools, including semantic find, typed interaction, get/check, screenshot/PDF evidence, console/errors/dialog/highlight/vitals, cookies/storage, network diagnostics/routes/HAR, plaintext state files, sessions/profiles, download/wait-download/upload, clipboard, tab/window, status, and skill helpers. |
...
| Debugging and observability | Partial | Console, errors, best-effort JavaScript dialog handling, highlight, best-effort Web Vitals, doctor, status, and session inspection are available; dashboard, trace, video, React DevTools introspection, and profiler are not. |
