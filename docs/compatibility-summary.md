...
| Debugging and observability | Partial | Console, errors, best-effort JavaScript dialog handling, highlight, best-effort Web Vitals, doctor, status, session inspection, and the local status/session dashboard are available; live viewport streaming, trace, video, React DevTools introspection, and profiler are not. |
