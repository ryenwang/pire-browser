...
| MCP agent integration | Supported | `pire-browser mcp --tools core` exposes the main browser workflow as stdio MCP tools, including semantic find, typed interaction, get/check, screenshot/PDF evidence, console/errors/dialog/highlight/vitals, network diagnostics/routes/HAR, plaintext state files, sessions/profiles, download/wait-download/upload, clipboard, tab/window, status, and skill helpers. |
