...
| Area | Status | Notes |
| --- | --- | --- |
| Install and setup | Supported | Global, project, and source installs are documented for the Firefox-backed package. `doctor --json` and `install-status --json` include concrete `nextActions`; `doctor --fix` provides an explicit setup repair path, and installed npm/Pi users can use `upgrade` or the lower-level `update` commands. |
