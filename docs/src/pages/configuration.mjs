...
PIRE_BROWSER_CONFIG=./ci-config.json pire-browser open https://example.com`),
  ...
  p("The packaged schema lives at <code>pire-browser.schema.json</code> in the repo and <code>./node_modules/pire-browser/pire-browser.schema.json</code> in an installed package. <code>restore: true</code> auto-loads and saves cookies plus origin-keyed localStorage using the selected session name; <code>restore: \"work\"</code> supplies an explicit key. <code>restoreSave: \"auto\"</code> protects a good state after failed import or validation. A profile name creates a temporary source snapshot, while a profile path is deliberately durable. <code>headless</code>, <code>args</code>, and <code>userAgent</code> apply only when launching a new session. <code>plugins</code> entries configure agent-browser-compatible credential providers and command/custom plugins."),
...
pire-browser set offline off`),
  ...
  table(["Variable", "Purpose"], [
    ...
    ["<code>PIRE_BROWSER_PROFILE</code>", "Firefox snapshot source name or deliberately persistent profile path."],
    ["<code>PIRE_BROWSER_SESSION</code>", "Default live session UUID or isolated session name."],
    ["<code>PIRE_BROWSER_NAMESPACE</code>", "Namespace for live sessions, temporary roots, and automatic restore directories."],
    ["<code>PIRE_BROWSER_SESSION_NAME</code>", "Deprecated session-plus-restore key alias."],
    ["<code>PIRE_BROWSER_RESTORE</code>", "Automatic cookie/localStorage restore assertion or explicit restore key."],
    ["<code>PIRE_BROWSER_RESTORE_SAVE</code>", "Automatic restore save policy: <code>auto</code>, <code>always</code>, or <code>never</code>."],
    ["<code>PIRE_BROWSER_RESTORE_CHECK_URL</code> / <code>_TEXT</code> / <code>_FN</code>", "Post-navigation restore validation checks."],
    ["<code>PIRE_BROWSER_AUTOSAVE_INTERVAL_MS</code>", "Minimum automatic restore save interval; <code>0</code> means close-only."],
    ["<code>PIRE_BROWSER_STATE_EXPIRE_DAYS</code>", "Automatic restore expiration age; default 30, <code>0</code> disables expiry."],
    ...
    ["<code>AGENT_BROWSER_NAMESPACE</code>", "Agent-browser-compatible alias for <code>PIRE_BROWSER_NAMESPACE</code>."],
    ...
    ["<code>AGENT_BROWSER_RESTORE_SAVE</code> / <code>RESTORE_CHECK_*</code> / <code>AUTOSAVE_INTERVAL_MS</code> / <code>STATE_EXPIRE_DAYS</code>", "Agent-browser-compatible restore lifecycle aliases."],
    ["<code>PIRE_BROWSER_STATE</code>", "Manual browser state file to preload before browser-control commands."],
    ...
    ["<code>PIRE_BROWSER_ARGS</code>", "Comma- or newline-separated Firefox launch arguments for newly launched managed sessions."],
    ...
    ["<code>PIRE_BROWSER_HIDE_SCROLLBARS</code>", "Hide native scrollbars during screenshots by default; set <code>0</code> or <code>false</code> to preserve them."],
    ["<code>AGENT_BROWSER_HIDE_SCROLLBARS</code>", "Agent-browser-compatible alias for screenshot scrollbar hiding."],
    ["<code>PIRE_BROWSER_DOWNLOAD_PATH</code>", "Explicit durable Firefox download directory; the default is temporary."],
    ["<code>AGENT_BROWSER_DOWNLOAD_PATH</code>", "Agent-browser-compatible alias for the durable download directory."],
