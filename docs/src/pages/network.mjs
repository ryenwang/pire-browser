...
pire-browser network requests --clear`),
  p("These are Firefox-backed guardrails, request-header helpers, network-idle waits, recent active-tab request diagnostics, and best-effort route interception. Route rules are scoped to the active tab. Body mocks use a WebExtension redirect, so they are useful for QA flows but are not full CDP response fulfillment."),
