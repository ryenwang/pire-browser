...
# device/set device is best-effort viewport + User-Agent/navigator/touch shim.
# Use open <url> --device <name> when the first request must see the mobile User-Agent.
# set offline is best-effort request blocking.
...
# Connect clients to ws://127.0.0.1:<port>/api/stream.
# Use record start/restart/stop when a screenshot-sequence evidence bundle is enough.`, "bash"),
  p("<code>stream enable</code> starts the dashboard-backed stream service in the background. <code>stream status --json</code> reports <code>transport: \"dashboard-websocket-screenshot\"</code>, <code>liveViewportKind: \"websocket-screenshot-stream\"</code>, <code>webSocketStreaming: true</code>, <code>remoteInput: true</code>, and a <code>webSocketUrl</code>. The stream sends screenshot-frame JSON and accepts basic mouse/keyboard/touch-shaped input events; it is not native WebM video or Chrome DevTools screencast output."),
...
pire-browser mcp --tools all`),
  p("The stdio MCP server exposes typed tools through profiles. Start with <code>core</code> for the inspect-before-act workflow, add comma-separated profiles such as <code>network</code>, <code>state</code>, <code>debug</code>, <code>tabs</code>, <code>mobile</code>, or <code>react</code> when needed, and use <code>all</code> only when the host can tolerate the full tool surface. Use <code>core,state</code> for configured plugin discovery through <code>pire_browser_plugin_list/show</code> before credential-provider login. Use debug-profile <code>pire_browser_install</code> only for explicit native-host setup or repair, and <code>pire_browser_upgrade</code> only for user-requested package update."),
