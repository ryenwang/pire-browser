...
pire-browser removeinitscript init1`),
  ...
  h2("Set page content", "set-page-content"),
  code(`pire-browser open
pire-browser setcontent '<main><h1>Hello</h1><button>Save</button></main>'
pire-browser snapshot -i`),
  p("<code>setcontent &lt;html&gt;</code> replaces the active page document HTML for a small fixture or reproduction. It is gated as an eval action and should be followed by <code>snapshot -i</code> before interacting. This is Firefox WebExtension document replacement, not CDP <code>Page.setDocumentContent</code>.")
];
