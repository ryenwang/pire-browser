...
pire-browser wait --download --timeout 60000`),
  p("Without <code>--download-path</code>, Firefox downloads live in the marked temporary session root and are removed after close. Use <code>--download-path &lt;dir&gt;</code>, <code>PIRE_BROWSER_DOWNLOAD_PATH</code>, or <code>AGENT_BROWSER_DOWNLOAD_PATH</code> for durable files. Relative paths resolve from the command's current working directory. With no explicit output path, <code>wait --download</code> reports the completed Firefox file path."),
  h2("Uploads", "uploads"),
