...
pire-browser deny c_8f3a1234`),
  ...
  p("Manual and automatic restore states may contain cookies, cross-origin session tokens, and localStorage secrets. They are plaintext by default for compatibility. Set <code>PIRE_BROWSER_ENCRYPTION_KEY</code> or <code>AGENT_BROWSER_ENCRYPTION_KEY</code> to a 64-character hex AES-256 key to write and load AES-256-GCM encrypted state files. Prefer <code>.pire-state/</code> for project files, keep keys out of logs and shell history, and use <code>state inspect --record</code> before loading sensitive state."),
  h2("Profile and cleanup boundary", "profile-cleanup-boundary"),
  p("Ordinary profiles and default downloads are created below a controlled OS temporary root. Cleanup requires a valid ownership marker and a canonical descendant path; directory names alone are not trusted. <code>doctor --fix</code> may remove only marked inactive orphans and expired automatic state. It never removes explicit <code>--profile &lt;path&gt;</code> directories or preserved 0.2.x profiles."),
  p("A named <code>--profile</code> is copied to a temporary snapshot and leaves the Firefox source unchanged. An explicit profile path is intentionally read-write and can contain passwords, cookies, IndexedDB, history, extensions, and cache. Keep durable profile paths outside repositories and do not share them."),
