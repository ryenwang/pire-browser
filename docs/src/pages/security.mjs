...
pire-browser --confirm-actions plugin:captcha:captcha.solve plugin run captcha captcha.solve --payload '{"siteKey":"abc"}'`),
  p("<code>plugin list</code> and <code>plugin show &lt;name&gt;</code> inspect configured agent-browser protocol plugins before running them; MCP users can use <code>pire_browser_plugin_list</code> and <code>pire_browser_plugin_show</code> from the state profile for the same discovery step. <code>auth login --credential-provider</code> runs a configured local plugin with capability <code>credential.read</code>. <code>plugin run &lt;name&gt; &lt;capability&gt; --payload &lt;json&gt;</code> runs plugins that declare <code>command.run</code> plus the requested custom capability. Keep vault tokens out of plugin args; use the vault vendor's own local session, keychain, or environment setup. Plugin stderr and plugin-provided error text are suppressed to reduce accidental secret exposure. Use <code>--confirm-actions plugin:&lt;name&gt;:&lt;capability&gt;</code> when provider or command-plugin access should require user approval before the plugin runs. <code>browser.provider</code> and <code>launch.mutate</code> are discoverable but not executed by this Firefox backend yet."),
];
