...
5. pire_browser_snapshot({ "interactive": true })`),
  ...
  p("Core schemas keep shared fields small. Session, namespace, restore, profile-source targeting, content boundaries, and output limits remain common. Put launch and policy options on <code>pire_browser_open</code>: state, local-file access, domain and action policy, confirmations, headless/headed mode, Firefox args, User-Agent, device, proxy, executable path, downloads, one-shot headers, and init scripts. Existing live sessions keep their launch context. Use the debug-profile <code>pire_browser_launch</code> only for lower-level launch diagnostics."),
  table(
    ["Field", "Purpose"],
    [
      ["namespace / session / restore / profile", "Compose an isolated live session, compact cookie/localStorage continuity, a named profile snapshot source, or an explicit durable profile path. sessionName remains a deprecated session-plus-restore alias."],
  ...
  table(
    ...
    [
      ...
      ["pire_browser_open / goto / navigate device field", "Apply a device preset before first navigation so the page request sees the mobile User-Agent and future page scripts see the navigator/touch shim."],
      ["pire_browser_set_viewport / pire_browser_device / pire_browser_set_device / pire_browser_set_geo / pire_browser_set_headers / pire_browser_set_credentials / pire_browser_set_media / pire_browser_set_offline", "Apply Firefox-backed settings and best-effort emulation controls. Prefer pire_browser_device for agent-browser-style device presets; set_device remains compatible. Device mode is viewport plus request User-Agent and page-level navigator/touch shim, not native mobile browser chrome or exact deviceScaleFactor."],
      ...
      ["pire_browser_state_*", "Save, load, list, show, inspect, rename, clear, or clean multi-origin cookie/localStorage state, including automatic restore entries. Legacy active-origin files remain readable; files are plaintext by default and optionally AES-256-GCM encrypted."],
      ["pire_browser_session_* / profiles_list / profiles_import", "Inspect isolated live sessions and Firefox profile sources. Named sources launch from temporary snapshots; explicit profile paths are durable. Imported and preserved 0.2.x sources remain explicit."],
      ["pire_browser_download / wait_download / upload", "Trigger or wait for browser downloads, or assign bounded local files to file inputs. Browser-initiated downloads are temporary unless open receives an explicit downloadPath."],
...
7. Use settings tools before screenshots or stateful QA when viewport, device preset, geolocation, headers, credentials, media, or offline mode matters. For first-navigation mobile checks, pass device on open/goto/navigate rather than setting it after the page has already loaded.
