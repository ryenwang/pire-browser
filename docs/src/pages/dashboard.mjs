...
pire-browser --auto-connect state save ./.pire-state/current.json`),
  p("Use the dashboard for a local summary, a live read-only preview that polls visible-viewport screenshots, and optional non-streaming dashboard chat powered by the same bounded AI Gateway command loop as <code>pire-browser chat</code>. The chat panel is enabled when <code>AI_GATEWAY_API_KEY</code> is present before the dashboard starts and forwards the currently previewed session when one exists. Use the CLI commands above when scripts or agents need structured output. <code>stream status --json</code> reports <code>transport: \"dashboard-http-polling\"</code> and <code>webSocketStreaming: false</code>. Activity is a bounded command log with secret-bearing arguments redacted; verify page success with snapshots, screenshots, URL checks, or other page state."),
  h2("Limits", "limits"),
