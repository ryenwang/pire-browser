...
pire-browser --auto-connect state save ./.pire-state/current.json`),
  p("Use the dashboard for a local summary and a live read-only preview that polls visible-viewport screenshots, and use the CLI commands above when scripts or agents need structured output. Activity is a bounded command log with secret-bearing arguments redacted; verify page success with snapshots, screenshots, URL checks, or other page state."),
  h2("Limits", "limits"),
  p("The preview is live and read-only, but it is polling screenshot frames rather than a WebSocket viewport stream. Dashboard-created sessions, remote input events, and native WebM/video recording remain future work in the Firefox backend. Use <code>record start</code> / <code>record stop</code> for screenshot-sequence recording."),
];
