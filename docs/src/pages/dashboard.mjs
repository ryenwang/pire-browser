...
pire-browser --auto-connect state save ./.pire-state/current.json`),
  p("Use the dashboard for a local summary, a polling preview of visible-viewport screenshots, screenshot-frame WebSocket streaming, and optional non-streaming dashboard chat powered by the same bounded AI Gateway command loop as <code>pire-browser chat</code>. The chat panel is enabled when <code>AI_GATEWAY_API_KEY</code> is present before the dashboard starts and forwards the currently previewed session when one exists. Use the CLI commands above when scripts or agents need structured output. <code>stream status --json</code> reports <code>transport: \"dashboard-websocket-screenshot\"</code>, <code>webSocketStreaming: true</code>, <code>remoteInput: true</code>, and <code>webSocketUrl</code>. Activity is a bounded command log with secret-bearing arguments redacted; verify page success with snapshots, screenshots, URL checks, or other page state."),
  h2("Limits", "limits"),
  p("The dashboard UI preview polls screenshot frames, while agent clients can connect to <code>ws://127.0.0.1:&lt;port&gt;/api/stream</code> for frame JSON and basic mouse/keyboard/touch-shaped input events. Dashboard chat currently returns a final response after the bounded loop finishes instead of streaming model tokens or step updates. Dashboard-created sessions, native WebM/video recording, and Chrome DevTools screencast output remain future work in the Firefox backend. Use <code>record start</code>, <code>record restart</code>, and <code>record stop</code> for screenshot-sequence recording."),
];
