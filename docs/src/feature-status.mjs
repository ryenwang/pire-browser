...
export const featureStatuses = {
  ...
  mcp: {
    status: "available",
    summary: "`pire-browser mcp --tools core` starts a one-page, 31-tool stdio profile for the default open, inspect, act, wait, and verify workflow. Agent-browser-style profiles `network`, `state`, `debug`, `tabs`, `mobile`, `react`, and `all` can be selected or comma-combined to expose broader tools and compatibility aliases only when needed. Core schemas keep shared targeting and output fields compact, while launch and policy options are typed on open. CI measures the installed skill, initialize response, core profile, and full profile against explicit byte/token budgets and verifies that the MCP server version matches the npm package. The server defaults to MCP protocol `2025-11-25`, accepts older supported client protocol versions during initialization, paginates larger profiles, and marks local maintenance/context tools as non-open-world for clearer host prompts.",
