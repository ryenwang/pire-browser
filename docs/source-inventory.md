...
| --- | --- | --- |
| `crates/pire-browser-core/` | Core Rust implementation | CLI parsing, launch/session lifecycle, IPC, install/status, Firefox integration, and shared protocol behavior. |
...
| `scripts/` | Maintainer automation | Install, package, smoke, RBXLX download, and oracle comparison workflows. |
| `fixtures/` | Test fixtures | Local HTML and oracle fixture data used by smoke/oracle tests. |
...
| `mvp.md`, `README.md`, `package.json`, `Cargo.toml` | Entry points | Product scope, usage, package scripts, and Rust workspace shape. |
