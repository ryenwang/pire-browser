...
| `pire-browser.schema.json` | Config schema | Authoritative packaged JSON Schema for `pire-browser` config defaults, launch args/User-Agent defaults, and credential-provider plugin entries; referenced from README and skill examples. |
| `agent-browser.schema.json` | Legacy config schema alias | Tracked filename alias for existing configs that reference the earlier schema path; mirrors the supported config keys including launch defaults and `plugins`. |
...
| `bin/pire-browser.js`, `scripts/platform.mjs`, `scripts/pi-install-migration.mjs`, `scripts/pi-postinstall.mjs` | Public npm launcher/install helpers | Root package launcher, platform resolver, Pi duplicate-source reconciliation helper including verified old managed GitHub checkout quarantine, and postinstall setup. |
