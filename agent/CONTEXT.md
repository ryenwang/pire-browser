...
- If your host supports MCP tools, start with `pire-browser mcp --tools core` for the core inspect, semantic find, interact, get/check, wait, evidence, diff, eval, status, basic tab, profile, close, and skill-guidance workflow. Add comma-separated profiles only when needed, such as `core,network`, `core,state`, `core,debug`, `core,tabs`, or `core,mobile`; use `all` only when the host can tolerate the full tool surface.
