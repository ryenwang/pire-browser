...
- If your host supports MCP tools, `pire-browser mcp --tools core` exposes typed tools for the core inspect, semantic find, interact, get/check, wait, evidence, debugging, network, plaintext state, session/profile, download/wait-download/upload, clipboard, tab/window, and close workflow through stdio.
