...
- For human-facing observability, `pire-browser dashboard start` opens a local status/session/profile/activity dashboard with a live read-only viewport preview that polls visible-viewport screenshots. Use `dashboard start --background`, `dashboard status`, and `dashboard stop` when the dashboard should outlive the launching shell. For machine-readable automation, keep using `status --json`, `doctor --json`, `session list --json`, `activity list --json`, and MCP tools.
