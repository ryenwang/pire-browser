...
4. If Pi reports a duplicate `pire-browser` tool from `npm:pire-browser` and an older GitHub, local-checkout, or legacy shim source, wait a moment and rerun `pi`; the npm postinstall helper removes old settings entries, old direct extension shims, and verified old Pi-managed GitHub checkouts after Pi records the npm source. If the conflict remains, remove the older source shown in Pi's error and then run `pi install npm:pire-browser`.
