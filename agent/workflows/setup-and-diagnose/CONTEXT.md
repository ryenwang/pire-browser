...
3. If postinstall was skipped by `--ignore-scripts` or an npm `allow-scripts` policy warning, run `pire-browser install` explicitly or retry the browser command that needs auto-launch.
