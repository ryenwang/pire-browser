...
- Use `pire-browser record start [output-dir] [url]`, `record status`, `record restart [output-dir] [url]`, and `record stop [output-dir]` for bounded visible-viewport PNG frames plus `recording.json`. This is screenshot-sequence QA evidence, not native WebM video, WebSocket viewport streaming, or Chrome DevTools screencast output.
