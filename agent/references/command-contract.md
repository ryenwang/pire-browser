...
- Use `pire-browser --session <name> --restore ...` for normal agent-browser-style project QA continuity. Named sessions persist through their managed Firefox profile. Use `pire-browser state save/load/list/show/inspect` only when an active-origin cookie/Web Storage artifact is needed. State files are plaintext by default; when `PIRE_BROWSER_ENCRYPTION_KEY` or `AGENT_BROWSER_ENCRYPTION_KEY` is set to a 64-character hex AES-256 key, saves write AES-256-GCM encrypted files and loads require the same key. Never print the key or cookie/storage values.
