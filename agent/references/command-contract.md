...
- Use `pire-browser record start`, `record status`, and `record stop [output-dir]` for bounded visible-viewport PNG frames plus `recording.json`. This is screenshot-sequence QA evidence, not native WebM video, live viewport streaming, or Chrome DevTools screencast output.
