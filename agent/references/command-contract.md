...
- Use `pire-browser open --enable react-devtools <url>`, `pire-browser react tree`, `pire-browser react inspect <fiberId|target>`, and `pire-browser react suspense --only-dynamic` for agent-browser-style React inspection. This is best-effort Firefox Fiber introspection, not full React DevTools render profiling; rerun `react tree` after route or DOM changes before reusing an `rN` id.
