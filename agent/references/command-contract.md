...
- Use `pire-browser profiles` before asking the user for Firefox profile paths. It lists managed profiles plus importable local Mozilla Firefox profiles discovered from `profiles.ini`. For logged-in app QA, derive `SESSION="$(pire-browser session id --scope worktree --prefix <app>)"`, run `pire-browser profiles import Default --name "$SESSION"` or import a discovered name/path into that same session name, then continue with `--session "$SESSION" --restore`. `Default` selects the discovered default Firefox profile when one is present. This copies the source profile, never mutates it, and future source changes do not sync. If the command reports `profile_in_use`, ask the user to close Firefox before retrying.
...
- Use `pire-browser --session <name> --restore ...` for normal agent-browser-style project QA continuity. Named sessions persist through their managed Firefox profile. Use `pire-browser session --json` for the current/default diagnostic and `pire-browser --session <name> --restore session info --json` to inspect selected session/profile/restore status; automation should read `data.restore.status` and `data.nextActions`. Use `pire-browser state save/load/list/show/inspect` only when an active-origin cookie/Web Storage artifact is needed. State files are plaintext by default; when `PIRE_BROWSER_ENCRYPTION_KEY` or `AGENT_BROWSER_ENCRYPTION_KEY` is set to a 64-character hex AES-256 key, saves write AES-256-GCM encrypted files and loads require the same key. Never print the key or cookie/storage values.
