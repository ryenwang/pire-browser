...
- `pire-browser profiles usage [<name>|--all]`
- `pire-browser profiles clean <name>|--all --dry-run|--yes`
- `pire-browser profiles delete <name> --yes`
...
- Unqualified npm/Pi package sources select npm's stable `latest` channel. `@beta` and `@rc` are explicit prerelease opt-ins; agents must not change channels without the user's request.
- `upgrade` and `update check/apply` derive a channel from the installed version. Stable follows `latest`; beta and RC follow their matching tags. Apply commands use the exact version returned by the registry and reject stale cached recommendations from another installed version or channel.
- Background `patch` mode may apply a stable patch release or a newer prerelease on the same channel. Minor/major updates require `upgrade`; a stable promotion is a separate channel choice.
- Return a direct npm install to stable with `npm install -g pire-browser@latest --include=optional`. For Pi, run `pi remove npm:pire-browser` and then `pi install npm:pire-browser`; Pi channel changes are remove-then-install, not `pi update`. Restart Pi afterward.
...
- Native `doctor --json` and `install-status --json` exit nonzero when `data.ok` is `false`; automation should inspect both `data.ok` and `data.nextActions` instead of treating a parseable JSON envelope as setup success. Missing or mismatched Native Messaging should report `pire-browser install` as the public setup repair command.
...
- Use `pire-browser setcontent <html>` only for small active-page fixtures or repros. It replaces the active page document HTML, is gated as `eval`, and should be followed by `snapshot` before interaction. It is Firefox WebExtension document replacement, not CDP `Page.setDocumentContent`.
...
- For a reproducible QA finding, use one stable session; start `trace`, optional `record`, and optional `network har start` before navigation or the interaction under test; capture the final screenshot, URL, compact snapshot, and targeted state; then stop HAR, recording, and trace in reverse order. Report artifact paths instead of pasting potentially sensitive evidence into chat.
...
- Use `pire-browser wait --fn <expression>` for short, side-effect-free page-world readiness predicates such as `window.appReady === true`; re-run `snapshot` before acting on refs after the wait.
...
- Use `pire-browser profiles` before asking the user for Firefox profile paths. It lists discovered Firefox sources, imported sources, and preserved 0.2.x profiles. `--profile <name>` copies the source without volatile caches into a temporary snapshot; it never mutates or continuously syncs the source. `--profile <path>` deliberately uses that path as a durable profile. If a source is locked, ask the user to close Firefox before retrying. Use `profiles usage`, `profiles clean`, and `profiles delete` for explicit legacy recovery; never delete profile directories by inference.
...
- Use `pire-browser --session <name> --restore ...` for normal project QA continuity. The session name selects an isolated live browser; `--restore` autosaves all profile cookies plus origin-keyed `localStorage` every 30 seconds while idle and on explicit close. Neither makes the Firefox profile durable. Use `--namespace <name>` to isolate live sessions, temporary roots, and restore keys. Use `pire-browser session --json` for the current/default diagnostic and `pire-browser --session <name> --restore session info --json` to inspect selected session/profile/restore status; automation should read `data.restore.status` and `data.nextActions`. Manual state schema v2 carries the same multi-origin cookie/localStorage model; legacy active-origin v1 files remain readable. State files are plaintext by default; when `PIRE_BROWSER_ENCRYPTION_KEY` or `AGENT_BROWSER_ENCRYPTION_KEY` is set to a 64-character hex AES-256 key, saves write AES-256-GCM encrypted files and loads require the same key. Never print the key or cookie/storage values.
- Without `--download-path`, browser-initiated downloads live under the temporary session root and are deleted with it. Use an explicit download path when the user expects a durable artifact.
...
- Use `pire-browser dialog status`, `dialog accept [text]`, and `dialog dismiss` when command output includes `PAGE_DIALOG` warnings. Dialog control is page-shimmed best effort; re-run `snapshot` after handling a dialog. Use global `--no-auto-dialog` or `AGENT_BROWSER_NO_AUTO_DIALOG=1` only for agent-browser-style debugging with the shim disabled; native Firefox page dialogs may block until handled manually.
