...
`vitals` is available for best-effort page performance checks. `react tree` and `react inspect` mirror agent-browser's React command shape using best-effort Firefox Fiber introspection. Full React DevTools render profiling and Suspense detail commands are not implemented yet.
...
`open --enable react-devtools` is accepted for agent-browser compatibility and reports that Firefox uses best-effort Fiber inspection rather than installing the full React DevTools hook. Re-run `react tree` after route changes or large DOM updates before using an `rN` id.
