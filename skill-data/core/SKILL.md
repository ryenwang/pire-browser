...
- `pire-browser doctor --json` and `pire-browser install-status --json` include `nextActions`; follow those concrete repair commands before guessing. If the platform-native optional package is missing, top-level help, `install --help`, `setup --help`, `doctor --help`, `install`, `setup`, and these diagnostic commands are still served by the JavaScript launcher and report the reinstall command with `--include=optional`.
