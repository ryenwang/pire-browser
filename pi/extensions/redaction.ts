...
export function redactProbe<T>(value: T): T {
  if (typeof value === "string") return redactDiagnosticText(value) as T;
  if (Array.isArray(value)) return value.map((item) => redactProbe(item)) as T;
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).map(([key, entry]) => [key, redactProbe(entry)])
    ) as T;
  }
  return value;
}
