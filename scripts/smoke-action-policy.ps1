...
$OriginalAllowedDomains = $env:AGENT_BROWSER_ALLOWED_DOMAINS
...
try {
  ...
  Remove-Item Env:AGENT_BROWSER_ALLOWED_DOMAINS -ErrorAction SilentlyContinue
  ...
  $env:AGENT_BROWSER_ACTION_POLICY = $DenyEvalPolicy
  ...
  $env:AGENT_BROWSER_ACTION_POLICY = $NavigateOnlyPolicy
  ...
  $null = Invoke-PireFailure "Unavailable command keeps NotAvailableError" @("--action-policy", $ReviewPolicy, "download", "#link", "out.zip", "--json") "NotAvailableError"
  ...
  if ($OriginalAllowedDomains) {
    $env:AGENT_BROWSER_ALLOWED_DOMAINS = $OriginalAllowedDomains
  } else {
    Remove-Item Env:AGENT_BROWSER_ALLOWED_DOMAINS -ErrorAction SilentlyContinue
  }
