...
pub const DOMAIN_POLICY_ENV_VAR: &str = "PIRE_BROWSER_ALLOWED_DOMAINS";
pub const LEGACY_DOMAIN_POLICY_ENV_VAR: &str = "AGENT_BROWSER_ALLOWED_DOMAINS";
...
pub fn collect_domain_policy() -> DomainPolicyDiagnostic {
    let env_value = domain_policy_env_value();
    domain_policy_from_env_value(env_value.as_deref())
}
...
pub fn resolve_domain_policy(args: &DomainPolicyArgs) -> Result<DomainPolicyDecision> {
    let env_value = domain_policy_env_value();
    resolve_domain_policy_from_env_value(env_value.as_deref(), args)
}

fn domain_policy_env_value() -> Option<String> {
    env_var_nonempty(DOMAIN_POLICY_ENV_VAR)
        .or_else(|| env_var_nonempty(LEGACY_DOMAIN_POLICY_ENV_VAR))
}
...
mod tests {
    ...
    fn shared_url_verdict_fixture_matches_rust_policy() {
        let fixture: UrlVerdictFixture = serde_json::from_str(include_str!(
            "../../../tests/fixtures/domain-policy-url-verdicts.json"
        ))
        .unwrap();
