...
fn profile_descriptors() -> Vec<McpProfileDescriptor> {
    vec![
        ...
        McpProfileDescriptor {
            name: "state",
            bits: PROFILE_STATE,
            description: "Cookies, storage, encrypted auth vault helpers, configured plugin discovery, plaintext or opt-in encrypted state files, sessions, profiles including Firefox profile import, typed clipboard tools, and skills.",
        },
        McpProfileDescriptor {
            name: "debug",
            bits: PROFILE_DEBUG,
...
fn tool_profile_bits(name: &str) -> u16 {
    match name {
        ...
        | "pire_browser_profiler_start"
        | "pire_browser_profiler_status"
        | "pire_browser_profiler_stop"
        | "pire_browser_record_start"
        | "pire_browser_record_status"
        | "pire_browser_record_stop"
        | "pire_browser_record_restart"
...
fn tool_command_args(
    ...
    match (profile, name) {
        ...
        (_, "pire_browser_profiler_start") => {
            args.push("profiler".to_string());
            args.push("start".to_string());
            if let Some(categories) = optional_string(object, "categories")? {
                args.push("--categories".to_string());
                args.push(categories);
            }
        }
        (_, "pire_browser_profiler_status") => {
            args.push("profiler".to_string());
            args.push("status".to_string());
        }
        (_, "pire_browser_profiler_stop") => {
            args.push("profiler".to_string());
            args.push("stop".to_string());
            if let Some(path) =
                optional_string(object, "path")?.or(optional_string(object, "outputPath")?)
        ...
        (_, "pire_browser_react_suspense") => {
            args.push("react".to_string());
            args.push("suspense".to_string());
            if optional_bool(object, "onlyDynamic")? {
...
fn push_headed_flags(
    args: &mut Vec<String>,
    object: &Map<String, Value>,
) -> std::result::Result<(), String> {
    let headless = optional_bool(object, "headless")?;
    let headed = optional_bool(object, "headed")?;
    if headless && headed {
        return Err("cannot use headless and headed together".to_string());
    }
    if headless {
...
fn core_tools() -> Vec<Value> {
    vec![
        ...
        tool(
            "pire_browser_upgrade",
            "Upgrade package",
            "Run the installed-package foreground upgrade path through the npm/Pi JavaScript launcher. Debug profile only; mutates the package install when the user requests a package update.",
        ...
        tool(
            "pire_browser_profiler_start",
            "Start profiler",
            "Start Firefox Performance Timeline trace-event profiling for the active tab.",
            tool_schema(
                vec![("categories", string_prop("Optional Chrome trace category CSV recorded as metadata only."))],
                &[],
            ),
            false,
        ),
        tool(
            "pire_browser_profiler_status",
            "Profiler status",
            "Report whether Firefox Performance Timeline profiling is active for the current tab.",
            tool_schema(vec![], &[]),
            true,
        ),
        tool(
            "pire_browser_profiler_stop",
            "Stop profiler",
            "Stop Firefox Performance Timeline profiling and optionally write Chrome Trace Event-shaped JSON to a path.",
            tool_schema(
                vec![
                    ("path", string_prop("Optional output JSON path.")),
                    ("outputPath", string_prop("Compatibility alias for path.")),
        ...
        tool(
            "pire_browser_record_start",
            "Start recording",
            "Start bounded Firefox screenshot-sequence evidence recording for the active tab; optionally set the later output directory and open a URL first.",
            tool_schema(
                vec![
                    ("outputDir", string_prop("Optional default output directory for a later bare record stop.")),
                    ("path", string_prop("Compatibility alias for outputDir.")),
                    ("url", string_prop("Optional URL to open before the first frame is captured.")),
        ...
        tool(
            "pire_browser_record_restart",
            "Restart recording",
            "Stop the active screenshot-sequence recording if present, then start another; optional outputDir/path and url apply to the new recording.",
            tool_schema(
                vec![
                    ("outputDir", string_prop("Optional default output directory for the new recording.")),
                    ("path", string_prop("Compatibility alias for outputDir.")),
                    ("url", string_prop("Optional URL to open before the new recording's first frame is captured.")),
                    ("intervalMs", number_prop("Frame interval in milliseconds, from 250 to 10000.")),
                    ("maxFrames", number_prop("Maximum frame count, from 1 to 120.")),
                ],
                &[],
        ...
        tool(
            ...
            tool_schema(
                vec![(
                    "onlyDynamic",
                    bool_prop("Only include currently fallback/dehydrated Suspense boundaries."),
                )],
                &[],
            ),
            true,
...
mod tests {
    ...
    fn lists_profile_specific_tools() {
        ...
        assert!(debug
            .iter()
            .any(|tool| tool["name"] == "pire_browser_profiler_start"));
        assert!(debug
            .iter()
            .any(|tool| tool["name"] == "pire_browser_profiler_status"));
        assert!(debug
            .iter()
            .any(|tool| tool["name"] == "pire_browser_profiler_stop"));
        assert!(debug
            .iter()
        ...
        assert!(debug
            .iter()
            .any(|tool| tool["name"] == "pire_browser_record_restart"));
        assert!(debug
            .iter()
    ...
    fn maps_tool_arguments_to_cli_args() {
        ...
        let args = tool_command_args(
            "pire_browser_profiler_start",
            &json!({ "categories": "devtools.timeline,v8.execute" }),
            McpToolsProfile::Debug,
        )
        .unwrap();
        assert_eq!(
        ...
        .unwrap();
        assert_eq!(args, vec!["--json", "profiler", "status"]);

        let args = tool_command_args(
            "pire_browser_profiler_stop",
            &json!({ "outputPath": "profile.json" }),
            McpToolsProfile::Debug,
        )
        .unwrap();
        assert_eq!(args, vec!["--json", "profiler", "stop", "profile.json"]);

        let args = tool_command_args(
            "pire_browser_record_start",
            &json!({ "outputDir": "recording-next", "url": "https://example.test", "intervalMs": 500, "maxFrames": 5 }),
            McpToolsProfile::Debug,
        )
        .unwrap();
        assert_eq!(
        ...
        let args = tool_command_args(
            "pire_browser_record_restart",
            &json!({ "path": "recording-next", "url": "https://example.test", "intervalMs": 750 }),
            McpToolsProfile::Debug,
        )
        .unwrap();
        assert_eq!(
        ...
        let args = tool_command_args(
            "pire_browser_react_suspense",
            &json!({ "onlyDynamic": true }),
            McpToolsProfile::React,
        )
        .unwrap();
        assert_eq!(args, vec!["--json", "react", "suspense", "--only-dynamic"]);
    ...
    fn rejects_invalid_tool_arguments() {
        ...
        let error = tool_command_args(
            "pire_browser_open",
            &json!({ "headless": true, "headed": true }),
            McpToolsProfile::Core,
        )
        .unwrap_err();
        assert!(error.contains("cannot use headless and headed together"));
