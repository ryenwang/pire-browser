...
pub const CONFIRM_ACTIONS_ENV_VAR: &str = "AGENT_BROWSER_CONFIRM_ACTIONS";
...
pub fn collect_confirmation_policy() -> ConfirmationPolicyDiagnostic {
    confirmation_policy_from_env_values(
        std::env::var(CONFIRM_ACTIONS_ENV_VAR).ok().as_deref(),
        std::env::var(CONFIRM_INTERACTIVE_ENV_VAR).ok().as_deref(),
    )
}
...
pub fn resolve_confirmation_policy(
    ...
    resolve_confirmation_policy_from_env_values(
        std::env::var(CONFIRM_ACTIONS_ENV_VAR).ok().as_deref(),
        std::env::var(CONFIRM_INTERACTIVE_ENV_VAR).ok().as_deref(),
        args,
    )
