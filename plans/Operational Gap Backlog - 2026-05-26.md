...
| Auth/token-flow ergonomics and secret-safe handoff | Epic 4 / Epic 7 | planned/in progress | Auth setup required one-time-token extraction and secret handling outside normal browser commands. The roadmap covers auth vault and credential safety, but the operator workflow needs an explicit safe handoff path. | Auth Handoff And Secret-Safe Diagnostics slice added profile-based auth handoff advisories; Named Session/Profile Lifecycle adds reusable `--session-name <name>` managed Firefox profiles for login-required workflows without implementing state export/import or the auth vault. |
